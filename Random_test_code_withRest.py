...
def main():
    ...
    rest = mat_data['eeg']['rest'][0][0]

    # Combine data and create labels
    X = np.vstack([movement_left, movement_right, rest])
    y = np.hstack([np.zeros(movement_left.shape[0]),
                   np.ones(movement_right.shape[0]),
                   np.full(rest.shape[0],2.)])
